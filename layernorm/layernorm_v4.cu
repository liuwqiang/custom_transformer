...
__global__ void mean_rstd_kernel(const float* inp, float* mean, float* rstd, int C) {
    ...
    if (blockDim.x >= 512) {
        if (tid < 256) {
            ...
            sumsq_shared[tid] += sumsq_shared[tid + 256];
        }
        __syncthreads();
    }
    if (blockDim.x >= 256) {
        if (tid < 128) {
            ...
            sumsq_shared[tid] += sumsq_shared[tid + 128];
        }
        __syncthreads();
    }
    if (blockDim.x >= 128) {
        if (tid < 64) {
            ...
            sumsq_shared[tid] += sumsq_shared[tid + 64];
        }
        __syncthreads();
    }

    if (tid < 32) {
        warpReduce(sum_shared, tid, blockDim.x);
        warpReduce(sumsq_shared, tid, blockDim.x);
    }
